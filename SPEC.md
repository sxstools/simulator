...
Status: **draft, v1 = proof of concept.** Items marked ⚠️ **TBD** are waiting on input (see [Open questions](#10-open-questions)). Full skill lists for Sage and the other classes will come after the proof of concept.
...
**Proof-of-concept pools.** v1 uses only the skills above:
- **Full-skill pools** (Rare / Epic / Legendary): the 22 skills with a known rarity.
- **Shard pool**: all 27 skills seen as shards. Shards are one flat 70% tier (§5), so the unknown rarities don't matter.

The complete Sage list and the other classes will be added after the proof of concept. The skill data should live in one data file (e.g. `skills.js`, keyed by class), so new lists can be dropped in without touching the app logic.
...
3. **Full skill lists** (§4): deferred until after the proof of concept, for Sage and the other classes.
